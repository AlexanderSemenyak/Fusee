...
namespace Fusee.Examples.Simple.Core
{
    ...
    public class Simple : RenderCanvas
    {
        ...
        private async Task Load()
        {
            ...
            //ищем максимальный Z у _rocketScene - обезь€нку пр€мо точно перед ракетов поставим
            ...
            //пробуем сместить координаты обезь€ны
            //1._monkeyScene содержит один child у себ€, но цикл все равно прогон€ем, вдруг будет больше одного
            ...
            foreach (SceneNode node in _speraScene.Children)
            {
                ...
                // {
                //   чилдов нет у обезь€нки, но могут быть в других модел€х. пока оставл€ем дл€ отладки
                // }
                //проходимс€ по компонентам каждого чилда
                //пока заметили три вида:
                //1.Transform - перенос, вращение, масштабирование
                //2.DefaultSurfaceEffect - ’«
                //3.Mesh - набор полигонов, составл€ющих объект
                ...
                foreach (var component in node.EnumComponents)
                {
                    ...
                    {
                        t.Translation += delta;
                        //переместим обезь€нку
                        //  t.Translation = new float3(0, 0, pointSphere.z + 7);
                        //  t.Translation = new float3(0, 0, (maxMonkeyZ - minMonkeyZ) / 2/*половинка обезь€ны по Z*/ - (maxRocketZ - minRocketZ) / 2/*половинка ракеты по Z*/);
                    ...
                    {
                        //Mesh - набор полигонов, составл€ющих объект
                        continue;
                    }
            ...
            sc.Children.AddRange(_speraScene.Children); //обезь€не надеваем на голову ракету 
        ...
        private float3 GetZForPointName(SceneContainer sceneContainer, string name)
        {
            foreach (SceneNode node in sceneContainer.Children)
            {
                if (node.Name == name)
                {
                    ...
                    var v0¬ернулиЌазад = v0Transformed * matrix.Invert();

                    var чистый—кейл = matrix.ScaleComponent();
                    var чистый“ранслэйт = matrix.TranslationComponent();
                    var чистый–отэйт = matrix.RotationComponent();
        ...
        private float FindMaxY(SceneContainer model, out float minY)
        {
            ...
            {
                //три набора элементов разного цвета дл€ ракеты (серый, зеленый, белый,....) - по каждому проходимс€ и ищем максимальную координату Z - она в мэшах
                foreach (SceneNode child in node.Children)
                {
                    //у каждого чилда есть Mesh и DefaultSurfaceEffect - нас интересуют меши - там точки, среди них и ищем максимальную координату z
        ...
        public override void RenderAFrame()
        {
            ...
            //переместим и смасштабируем головоу обезь€нки
            ...
            //float4x4 mtxOffset = float4x4.CreateTranslation(0, 0, -0.3f); //трансофрмаци€ - перемещение по 20 точек по x,y,z
