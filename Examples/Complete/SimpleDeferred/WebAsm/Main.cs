...
namespace Fusee.Examples.SimpleDeferred.Main
{
                ...
                new AssetHandler
                {
                    ...
                    DecoderAsync = async (string id, object storage) =>
                    {
                        if (Path.GetExtension(id).IndexOf("fus", System.StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            ...
                            return await Task.Factory.StartNew(() => new ConvertSceneGraph().Convert(ProtoBuf.Serializer.Deserialize<SceneContainer>(storageStream)));
                        }
                        return null;
                    },
                    Checker = (string id) =>
